...
public class Hand {
	...
	public Hand EvaluateHand() throws HandException{
		...
		int H = this.getCardsInHand().size();
		if (H != 5){
	...
	public static ArrayList<Hand> ExplodeHands(Hand h) {
		...
		ArrayList<Hand> CompileHands = new ArrayList<Hand>();
		ArrayList<Card> GatherCards = new ArrayList<Card>();
		for(Card J : h.getCardsInHand()){
			if (J.geteRank() != eRank.JOKER){
				ReturnHands.add(h);
			}
			if (J.geteRank() == eRank.JOKER){
				for(eRank r : eRank.values()){
					for(eSuit s : eSuit.values()){
						GatherCards.add(new Card(r, s, 1));
						for(Card card : GatherCards){
							J = GatherCards.get(GatherCards.size()-1);
							h.AddCardToHand(J);
							CompileHands.add(h);
							h.getCardsInHand().remove(J);
							}
						}
					}
				}ReturnHands.addAll(CompileHands);
			}
		return ReturnHands;
	}
